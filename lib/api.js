...
export const fetchUserProfile = async (userId) => {
    ...
    const { data, error } = await supabase
        .from('profiles')
        .select(`username, total_points, score_daily, score_weekly, daily_goal, streak_count, best_streak, created_at, native_lang, app_lang, is_admin, is_premium, last_score_date, current_level`)
        .eq('id', userId)
        .single();
...
export const updateUserPointTransaction = async (userId, delta) => {
    ...
    // Using 'en-CA' is a reliable hack for YYYY-MM-DD format universally in JS
...
export const fetchUserStats = async (userId) => {
    // Validate UUID format to prevent 400 RPC error
    const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
    if (!userId || !uuidRegex.test(userId)) {
        console.warn("fetchUserStats called with invalid user ID:", userId);
        return { questions_seen: 0, words_seen: 0 };
    }
...
export const fetchDetailedStats = async (userId) => {
    try {
        const { data, error } = await supabase.rpc('get_detailed_stats', { p_user_id: userId });
        if (error) throw error;
        return data;
    } catch (e) {
        console.warn("fetchDetailedStats failed (RPC likely missing):", e.message);
...
// --- REPORT FUNCTION ---
export const submitQuestionReport = async (userId, questionId, topic, message) => {
    const { error } = await supabase
        .from('reports')
        .insert({
            user_id: userId,
